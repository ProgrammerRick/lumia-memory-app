 ...
 *
 * Campos principais (Etapa 3): título, descrição, data, categoria,
 * sentimento e imagem de capa.
 *
 * Campos de mídia (Etapa 4): uma memória agora pode carregar uma lista de
 * itens de mídia (`media`) — imagens, vídeos e áudios — além da imagem de
 * capa já existente. Cada item é guardado localmente como uma data URL
 * (base64), no mesmo espírito da `coverImage`.
 *
...
export interface Memory {
  ...
  coverImage?: string;
  /** Itens de mídia (fotos, vídeos, áudios) guardados nesta memória — Etapa 4. */
  media?: MediaItem[];
  ...
  /** Reservado (uso legado; a Etapa 4 usa o array `media` acima). */
  videoUrl?: string;
  /** Reservado (uso legado; a Etapa 4 usa o array `media` acima). */
  audioUrl?: string;
...
/** Tipos de mídia suportados por uma memória — Etapa 4. */
