...
>
> **A Etapa 5 não alterou nenhum token, cor, fonte, raio, sombra ou animação
> deste documento.** As únicas adições foram comportamentais (ver seção 14).
...
- `Logo`, `Button` (variantes `primary`, `glass`, `ghost`, `danger`; ganhou a
  prop comportamental `loading` na Etapa 5), `GlassCard`, `IconButton`,
  `ScreenHeader` (com slot `leading` opcional), `EmptyState`, `Chip`,
...
- `MemoryCardSkeleton` *(novo — Etapa 5)*: placeholder de carregamento com a
  mesma silhueta exata do `MemoryCard`, usando apenas os tokens já
  existentes (`glass-panel`, `rounded-3xl`, `bg-white/[0.06]`).
...
## 14. Comportamento e interação (novo — Etapa 5)
...
- **Estado de sucesso/erro**: o `ToastContext` ganhou os tipos `media` e
  `error` (mesmo componente visual `glass-panel-strong`, apenas novos ícones
  Lucide) para cobrir mais ações sem criar um novo padrão de feedback.
- **Foco de teclado**: `:focus-visible` global (só aparece em navegação por
  ...
  do mouse/touch.
- **Áreas clicáveis**: cartões e itens de lista com ação ganharam
