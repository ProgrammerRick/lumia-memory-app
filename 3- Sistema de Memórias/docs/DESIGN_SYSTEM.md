...
- Entrada de tela: fade + leve translação vertical (8–16px) + leve escala
  (0.98–0.99 → 1), 0.35–0.6s.
- Transição entre telas (`ScreenTransition`): fade + slide + scale sutil,
  `mode="wait"` no `AnimatePresence`, sem transições bruscas ou cortes secos.
- Botões (`Button`, `IconButton`): `whileHover={{ scale: 1.02–1.06 }}`,
  `whileTap={{ scale: 0.92–0.98 }}`, acompanhados de um **ripple discreto**
  (círculo translúcido que se expande a partir do ponto de toque e some em
  ~0.6s) — nunca um ripple chamativo ou colorido.
- Indicador ativo da navegação: `layoutId` do Framer Motion para deslizar
  suavemente entre abas (ver `BottomNav`), com leve brilho (`drop-shadow`)
  no ícone ativo.
- Elementos "vivos" (logo, ícones de estado vazio): animação flutuante lenta
  (`--animate-float`, 6–7s, `ease-in-out`, `infinite`), pulsação suave
  (`--animate-pulse-slow`) e, no caso do `Logo`, um halo cônico com rotação
  muito lenta (≥20s) para sugerir "chama viva" sem chamar atenção.
- Carregamento inicial (`AppLoader` em `App.tsx`): véu breve (~0.9s) com o
  logo pulsando suavemente antes de revelar a tela Welcome — substitui
  qualquer "flash" abrupto de conteúdo.
...
- O `AppShell` aplica uma textura de ruído/grão muito discreta (opacidade
  ~0.05, `mix-blend-overlay`) sobre toda a "vitrine" do app, reforçando a
  sensação nostálgica de filme fotográfico sem comprometer a legibilidade.

## 12. Componentes do Design System (já implementados)

Local: `src/components/ui/`

- `Logo` — marca do app (orbe de luz animado com halo cônico sutil).
- `Button` — botão base com 3 variantes: `primary`, `glass`, `ghost`, com
  ripple discreto de toque.
- `GlassCard` — cartão de vidro reutilizável, com variante `strong` e modo
  `interactive` (elevação e brilho de borda no hover).
- `IconButton` — botão circular apenas com ícone, com ripple discreto.
- `ScreenHeader` — cabeçalho padrão de tela (eyebrow + título serif + subtítulo).
- `EmptyState` — estado vazio ilustrado (ícone flutuante com glow + título + descrição).

Local: `src/components/layout/`

- `AppShell` — moldura raiz do app (glow ambiente, friso de luz e grão sutil,
  vitrine em formato de telefone em telas grandes).
- `BottomNav` — navegação inferior fixa com indicador animado, brilho no
  ícone ativo e botão de destaque central ("Criar") com glow.
- `ScreenTransition` — wrapper de transição de entrada/saída de tela
  (fade + slide + scale sutil).
