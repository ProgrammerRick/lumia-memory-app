/**
 * Telas principais do aplicativo Lumia.
 *
 * "welcome" é a tela de entrada (splash/onboarding inicial).
 * As demais são as telas do app após o usuário entrar.
 *
 * "memory-detail" foi adicionada na Etapa 3 (Sistema de Memórias): é uma
 * tela de destino (não aparece na navegação inferior), aberta a partir de
 * um cartão de memória na Home ou na Timeline.
 */
export type AppScreen =
  | "welcome"
  | "home"
  | "create"
  | "timeline"
  | "settings"
  | "memory-detail";

/**
 * Telas que possuem um item correspondente na navegação inferior.
 * "welcome" e "memory-detail" ficam de fora propositalmente: a primeira é
 * uma tela de entrada única, a segunda é uma tela de destino contextual.
 */
export type TabScreen = Extract<AppScreen, "home" | "create" | "timeline" | "settings">;

export const TAB_SCREENS: TabScreen[] = ["home", "create", "timeline", "settings"];

/**
 * Parâmetros opcionais que uma navegação pode carregar.
 *
 * Modelado de forma genérica para crescer sem quebrar a navegação
 * existente: hoje carrega apenas `memoryId` (usado por "memory-detail" e
 * pelo modo de edição da tela "create"), mas pode ganhar novos campos no
 * futuro (ex.: aba inicial de um filtro, origem da navegação, etc.).
 */
export interface NavigationParams {
  memoryId?: string;
}
